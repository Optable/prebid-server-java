...
public class OptableAttributesResolver {
    ...
    public OptableAttributes reloveAttributes(AuctionContext auctionContext, Long timeout) {
        final String ip = ipResolver.resolveIp(auctionContext);
        ...
        return optableAttributes != null
                ? optableAttributes.toBuilder().ip(ip).timeout(timeout).build()
                : OptableAttributes.builder().ip(ip).timeout(timeout).build();
    }

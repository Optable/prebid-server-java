...
public class APIClient {
    ...
    public Future<TargetingResult> getTargeting(String query, String ip, long timeout) {
        logger.debug("Query string: %s".formatted(query));

        final MultiMap headers = HeadersMultiMap.headers()
                .add(ACCEPT_HEADER, "application/json");

        if (StringUtils.isNotEmpty(apiKey)) {
            headers.add(AUTHORIZATION_HEADER, "Bearer %s".formatted(apiKey));
        }

        if (StringUtils.isNotEmpty(ip)) {
            headers.add(X_FORWARDED_FOR_HEADER, ip);
        }
